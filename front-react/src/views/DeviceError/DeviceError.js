...
const DeviceError = () => {
  ...
  async function getAccesses () {
    ...
    setAccesses(result.data.filter(i => i.create_dt.split(' ')[0] === date[0]))
    setLoading(false)
  }
  ...
  const handleDate = (val) => {
    setDate(val);
    if(val[0] === val[1]) {
      setAccesses(originAcc.filter(i => i.create_dt.split(' ')[0] === val[0]))
      setAllAcc(originAcc.filter(i => i.create_dt.split(' ')[0] === val[0]))
    } else {
      setAccesses(originAcc.filter(i => i.create_dt.split(' ')[0] >= val[0] && i.create_dt.split(' ')[0] <= val[1]))
  ...
  return (
    <div className={classes.root}>
      <Card className={classes.root,classes.cardcontent}>
        <UsersToolbar className={classes.toolbar} dateChange={handleDate}/>
