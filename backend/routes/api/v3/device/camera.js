...
let qrystr = [
  {
    ...
    c_license_key2:
      "U2FsdGVkX1/pcs7DTGoQeyuc6yqEuPw4Rk+wrhHKs4aJ7S7ksy3L/eOAL135NgQT",
  },
];
  ...
  if (data.c_type === "off") {
    ...
    if (!file) {
      ...
      if (server_mac_check(val.license_eth) != val.info[0].mac) {
        return {
          result: false,
          msg: "올바르지 않은 맥주소 입니다 라이센스를 다시 등록해주세요.",
        };
      }
...
async function license_check(type, req) {
  ...
  if (!token_result) return { result: false, msg: "유효하지 않느 토큰입니다." };
    ...
    if (qryResult.length < 1) {
      ...
      if (val.c_type === "off") {
        ...
        if (qrystr.length < 1) {
          ...
          return {
            result: false,
            msg: "올바르지 않은 라이센스 입니다 라이센스를 다시 등록해주세요.",
          };
        } else {
          let _val = qrystr[0];
...
async function license_file_write(data) {
  ...
  if (file) {
    ...
    if (!res) {
      return {
        result: false,
        msg: "외부에 접속 할 수 있도록 인터넷을 설정해주세요.",
      };
    } else {
...
router.post("/license_save", async function (req, res) {
  try {
    ...
    if (result.length > 0) {
      await api_v3_device_license.updateOne(
        { _id: result[0]._id },
        {
...
router.get("/", async function (req, res) {
  try {
    let get_data;
    let auth =
      req.query.authority === "admin"
        ? new RegExp("")
        : new RegExp("^" + req.query.authority + "$");
    if (req.query.authority.split("-").length === 2) {
      auth = new RegExp("^" + req.query.authority);
    }

    if (req.query.authority === "admin") {
      get_data = await api_v3_device_camera
        .find()
        .populate("gateway_obid")
        .sort("status");
    } else {
      get_data = await api_v3_device_camera
        .find()
...
router.put("/:id", async function (req, res) {
  try {
    ...
    const update = await api_v3_device_camera.findByIdAndUpdate(
      id,
      update_data,
      { new: true }
    );
...
router.delete("/:id", async function (req, res) {
  try {
    ...
    if (req.body.devices === undefined) {
      ...
      let operations = [];
      let result = await api_v3_device_camera.deleteMany({
